...
for s in samples:
    ...
    try:
        #with open(glob.iglob(os.path.join(opts.input,s,"*.gtf")).next()) as f: #python2
...
for q, s in enumerate(samples):
    ...
    try:
        #with open(glob.iglob(os.path.join(opts.input,s,"*.gtf")).next()) as f: #grabs first .gtf file it finds inside the sample subdirectory #python2
        with open(next(glob.iglob(os.path.join(opts.input,s,"*.gtf")))) as f:
